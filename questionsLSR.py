...
[all_docs, labels, counts] = load_docs('BigDataset.tsv', vocab, map,10000)
...
yTrain_c = np_utils.to_categorical(Ytrain, 4)

#LSR
eps = 0.4
for i in range(0,len(yTrain_c)):
	for j in range(0,4):
		x = yTrain_c[i][j]
		yTrain_c[i][j] = x * (1 - eps) + (1-x) * eps / (4-1)
...
callback = callbacks.EarlyStopping(monitor='val_acc',
                              ...
                              patience=0,
                              verbose=0, mode='auto')
history = nn.fit(Xtrain, yTrain_c, epochs=1, validation_split=0.10)
